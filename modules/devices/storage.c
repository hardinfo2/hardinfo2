...
gboolean __scan_udisks2_devices(void) {
	...
	gchar is_emmc=0;
        ...
        udisks2_storage_list = h_strdup_cprintf("$%s$%s=%s|%s|%s %s\n", udisks2_storage_list, devid, disk->block_dev, size, (disk->serial ? disk->serial : ""), (ven_tag ? ven_tag : ""), disk->model);
		...
		if ((prcap = popen(tmp, "r"))) {
			...
			if (g_str_has_prefix(buf, "  Does")) {
				...
				capab =
				   ...
				   && strstr(buf, "kB/s")) {
			    speed =
				g_strconcat(speed ? speed : "", strreplace_chr(g_strstrip(buf), ':', '='), "\n", NULL);
